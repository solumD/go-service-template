...
func NewEntityRepository(postgres *postgres.Postgres) *entityRepository {
...
func (r *entityRepository) CreateEntity(ctx context.Context, entity *model.Entity) (int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("failed to create entity: %v (name: %s)", err, entity.Name)
	}
...
func (r *entityRepository) GetEntityByID(ctx context.Context, id int) (*model.Entity, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get entity: %v (id: %d)", err, id)
	}
